...
class _StudentScheduleScreenState extends State<StudentScheduleScreen> {
  ...
  Future<List<List<Map<String, dynamic>>>> fetchSchedules() async {
    final url = Uri.parse('http://192.168.254.103:5000/schedules/${widget.userId}');
            ...
            for (var i = 0; i < yearSchedules.length; i++)
              Column(
                ...
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SingleChildScrollView(
                          ...
                          child: DataTable(
                            columns: [
                              DataColumn(label: Text('Subject Code', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Description', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Day', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Time', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Room', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Faculty', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('Class Type', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))),
                            ],
                            rows: yearSchedules[i]['schedules'].map<DataRow>((schedule) {
                              return DataRow(cells: [
                                DataCell(Text(schedule['subject_code'].toString())),
                                DataCell(Text(schedule['description'].toString())),
                                ...
                                DataCell(Text(schedule['class_type'].toString())),
                              ]);
                            }).toList(),
                          ),
                        ),
                  ...
                  if (i < yearSchedules.length - 1) SizedBox(height: 16), // Add SizedBox between semesters except for the last one
                ],
              ),
            SizedBox(height: 100), // Adjust the final SizedBox height as per your requirement
          ],
        );
      },
    );
    ...
    return Theme(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            'Student Schedule',
            style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold, fontFamily: 'Times New Roman', color: Colors.white),
          ),
          toolbarHeight: 70,
          flexibleSpace: Stack(
            children: [
